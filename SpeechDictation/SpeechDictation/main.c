...
void run_iat(const char* audio_file, const char* session_begin_params)
{
	...
	printf("=============================================================\n");
	if (rec_result[0] == -66 && rec_result[1] == -56 && rec_result[2] == -61 && rec_result[3] == -4)
	{
		printf("bingo!\n");
		printf("=============================================================\n");
	}
...
int main(int argc, char* argv[])
{
	...
	const char* login_params = "appid = 577b1c89, work_dir = ."; // 登录参数，appid与msc库绑定,请勿随意改动
	...
	run_iat("wav/2017.wav", session_begin_params); //iflytek02音频内容为“中美数控”；如果上传了用户词表，识别结果为：“中美速控”。
exit:
	printf("按任意键退出 ...\n");
	_getch();
	MSPLogout(); //退出登录
